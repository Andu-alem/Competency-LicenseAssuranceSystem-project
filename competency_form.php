...
<html>
	...
	<body>	
		<?php	
			...
			if (isset($_REQUEST['full_name'])){
				...
				if (!$cond) {
					...
					echo "<script>alert('you registered latter')window.location=\'delete_catagory.php\';</script>";
				}
				else{ 		
					...
					$inst_quer= "INSERT INTO `address`(region,zone,wereda,kebele,house_no,telephone,p_o_box,fax,email) VALUES('$rg','$zn','$wd','$kl','$hn','$tp','$pobx','$fx','$eml')";
					$res= mysqli_query($con,$inst_quer) ;
					if ( $res and $result) {
						echo '<script type="text/javascript">alert("your info is recoreded successfully !");window.location=\'view_request.php\';</script>';
		...
		<fieldset style=" background-size:cover; background-color:white; "> <legend style="color: white;"> </legend>
			<div class="form"  > 			
